...
Ninputs   = 18;  %40     % Number of inputs/antennas
Nbins     = 96;  %500;   % Total number of frequency bins
...
% time samples.
D = 5; % DM; 10 with these parameters gives a fairly fast pulsar
...
%%
figure(10); 
imagesc(squeeze(abs(pulseData(5,:,:))));
% imagesc(abs(exp(1j*phi)));
Nbeam = 14;
wei = ones(Ninputs,Nbins,Nbeam);
bf_data = zeros(Ntime,Nbins,Nbeam);
for b = 1:Nbeam
    for f = 1:Nbins
        w = wei(:,f,b);
...
Nmcnts = 50;
bf_sti = zeros(Ntime_per_packet,Nbin_per_x,Nbeam);
for k = 1:Ntime_per_packet
    bf_sti(k,:,:) = mean(bf_data(1+(k-1)*Nmcnts:k*Nmcnts,1:Nbin_per_x,:),1);
end

figure(12);
imagesc(10*log10(abs(bf_data(:,:,6))).');
title('Simulated pulsar output (96 bins)');
ylabel('Frequency bin index');
xlabel('Time samples')

figure(13);
imagesc(10*log10(abs(bf_sti(:,:,6))).');
title('Simulated pulsar STI output (8 bins)');
ylabel('Frequency bin index');
xlabel('Time samples')