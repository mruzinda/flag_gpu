...
// First 56 bits are the mcnt (system time index)
// Next 4 bits are the Fengine ID from which the packet came
// Next 4 bits are the Xengine ID to which the packet is destined
//////Not relevant to ONR //////////////////////////////////////////
...
static inline int get_block_idx(uint64_t mcnt) {
    ...
    // If N_INPUT_BLOCKS = 20, Nm = 50 and mcnt is an iterating index, then
    // when mcnt reaches 50, the function returns a 1, mcnt = 100, the 
    // function returns a 2 and so on until we get to 20 the N_INPUT_BLOCKS
    // then it starts over.
}
...
//	           pkt_header->mcnt, pkt_header->mcnt-prior_mcnt, pkt_header->cal, pkt_header->fid, pkt_header->xid);
    ...
    // MR: Changed to ONR packetizer without 'cal' value (version 2) ////////
...
static inline void get_header (struct hashpipe_udp_packet *p, packet_header_t * pkt_header) {
    ...
    // MR: Changed to match  ONR packet header ////////////////////////////
    //pkt_header->mcnt        = raw_header >> 20;
    //pkt_header->cal         = (raw_header >> 8) & 0x00000000000003FF;
...
static inline void initialize_block(flag_input_databuf_t * db, uint64_t mcnt) {
    ...
    // MR: Initializing data array to 0 to solve 6 input issue
    //db->block[block_idx].data[N_BYTES_PER_BLOCK/sizeof(uint64_t)] = 0;
}
    ...
    while (run_threads()) {
        ...
        if (cur_state == ACQUIRE) {
            ...
            do {
                ...
                //fprintf(stderr, "NET: 633 MASTER CMD = %d\n", master_cmd);

                p.packet_size = recv(up.sock, p.data, HASHPIPE_MAX_PACKET_SIZE, 0);
                
                //fprintf(stderr, "NET: 637 MASTER CMD = %d\n", master_cmd);
