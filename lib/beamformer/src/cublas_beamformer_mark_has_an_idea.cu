...
//	}
	...
	//	int out_idx = i + Ni*f + Nf*Ni*t + Nt*Nf*Ni*m + Nm*Nt*Nf*Ni*c;
	//	
	//	data_restruc[out_idx].x = 0*1.0f;
	//	data_restruc[out_idx].y = 0*1.0f;
	//}
...
void run_beamformer(signed char * data_in, float * data_out) {
	...
	int Nm = 50; // 200;
	int Nf = 4; // 8; // 32; //8; // 3; // 8 because # of fengines * # of inputs = 8*8 = 64 (optimized for gpu).
	int Nt = 85; // 20;
	int Nc = 8; // 25;
	int Ni = 6; // 6; // 8; // 6; // 8; because # of fengines * # of inputs = 8*8 = 64 (optimized for gpu)
